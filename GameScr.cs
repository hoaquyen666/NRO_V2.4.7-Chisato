...
public class GameScr : mScreen, IChatable
{
	...
	public static Image avtMiyo = mSystem.loadImage("/mainImage/avtMiyo.png");

	public const int ID_avtMiyo = 999999999;
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if (CommandKey.KeyPress(GameCanvas.keyAsciiPress)) { }
					}
    ...
    public static System.Collections.Generic.List<System.Action> runOnMainThread = new System.Collections.Generic.List<System.Action>();
    public override void update()
	{
        lock (runOnMainThread)
        {
            while (runOnMainThread.Count > 0)
            {
                try
                {
                    System.Action action = runOnMainThread[0];
