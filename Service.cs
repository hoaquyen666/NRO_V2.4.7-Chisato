...
public class Service
{
    ...
    public void sendMove(int cx, int cy)
    {
        int num = cx - Char.myCharz().cxSend;
        int num2 = cy - Char.myCharz().cySend;
        if (Char.ischangingMap || (num == 0 && num2 == 0) || Controller.isStopReadMessage || Char.myCharz().isTeleport || cy <= 0 || Char.myCharz().telePortSkill)
        {
            return;
        }
        try
        {
            Message message = new Message((sbyte)(-7));
            Char.myCharz().cxSend = cx;
            Char.myCharz().cySend = cy;
            Char.myCharz().cdirSend = Char.myCharz().cdir;
            Char.myCharz().cactFirst = Char.myCharz().statusMe;
            ...
            message.writer().writeShort(cx);
            if (num2 != 0)
            {
                message.writer().writeShort(cy);
            }
            session.sendMessage(message);
            GameScr.tickMove++;
            message.cleanup();
	...
	{
		if (CommandChat.OnChat(text)) return;
            Message message = null;
		try
		{
			message = new Message((sbyte)44);
